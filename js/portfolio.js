/* ============================================================
   온종일 · portfolio.js
   저장: localStorage | 이미지: Cloudinary 업로드 or URL 자동추출 or 이모지
   ============================================================ */
...
let editingId = null;
let currentThumbMode = 'auto'; // 'auto' | 'emoji'
...
function renderPortfolio(){
  ...
  portfolio.forEach((p,i)=>{
    ...
    `;
    card.querySelector('.js-edit').addEventListener('click', e=>{ e.stopPropagation(); editPortfolio(p.id); });
  ...
  if(isFull){
    ...
    addCard.onclick = ()=>alert('포트폴리오 등록 한도(20개)에 도달했습니다.');
  } else {
...
function showThumbPreview(imgUrl, badge){
  ...
  document.getElementById('thumbPreview').innerHTML         =
    `<img src="${imgUrl}" alt="미리보기" onerror="showThumbError('이미지를 불러올 수 없어요.')">`;
...
function onFileUpload(event){
  ...
  reader.onload = e => showThumbPreview(e.target.result, '📁 업로드 중...');
...
async function uploadToCloudinary(file){
  ...
  try {
    ...
    if(data.secure_url){
      showThumbPreview(data.secure_url, '☁️ Cloudinary');
    } else {
